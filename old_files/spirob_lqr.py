...
def controller(model, data, invariants, previous_solution=None, S=None):
    ...
    u = np.linalg.pinv(Bp, rcond=1e-6) @ tau

    try:
        data.ctrl = np.minimum(u, 0.0) # Tendon pulling only
...
def simulate_model(headless=False, record_video=False, video_fps=30):
    ...
    last_ctrl = time.time()
    max_sim_time = 125.0  # Run for 25 seconds of simulation time
    ...
    if headless:
        ...
        while data.time < max_sim_time:
            ...
            task_error, q, dq, previous_solution = controller(model, data, invariants, previous_solution, S = None)
