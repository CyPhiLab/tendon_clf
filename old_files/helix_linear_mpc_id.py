...
def controller(model, data, invariants, previous_solution=None):
    ...
    # data.mocap_pos[mocap_id] = ([-0.1,   0.0,  0.4])
    # data.mocap_pos[mocap_id] = ([-0.2,   0.0,  0.5])
    # data.mocap_pos[mocap_id] = ([-0.3,   0.0,  0.4])
    # data.mocap_pos[mocap_id] = ([-0.2,   0.0,  0.3])    
    ...
    for k in range(N - 1):
        ...
        jac = jac + dt * dJ_dt  # approximate jacobian change over time
        constraints += [pinv_B @ (M @ qdd_k[:, k:k+1] + C @ dq + g.reshape(-1,1) + data.qfrc_passive.reshape(-1,1)) == u_k[:, k:k+1]]
    ...
    try:
        ...
        if u_k.value is not None:
            ...
            # print(f"converged\n")
            return task_error, q, dq, current_solution, u_k.value[:, 0]
        else:
            print(f"failed convergence - no solution\n")
            return task_error, q, dq, previous_solution, u_k.value[:, 0]
...
def simulate_model(headless=False):
    ...
    if headless:
        ...
        while (len(task_error_log) < 2 or
            ...
            task_error, q, dq, previous_solution, u = controller(model, data, invariants, previous_solution)
