...
def controller(model, data, invariants, previous_solution=None):
    ...
    # data.mocap_pos[mocap_id] = np.array([0.05, 0.0, 0.25])
    ...
    K = 500
    mu_des = (K * twist.reshape(-1,1)  - 2 * np.sqrt(K) * (jac @ dq))
    N = np.eye(model.nv) - np.linalg.pinv(jac) @ jac
    qdd_null = N @ qdd

    # ----------------------------
    # 1) Null reference (stabilizing)
    # ----------------------------
    Knull = 1000.0
    Dnull = 2 * np.sqrt(Knull)
    q = data.qpos.reshape(-1,1)
    # ----------------------------
    # 2) Zero reference
    delta_q = cp.Variable(shape=(model.nv, 1))
    objective = cp.Minimize(cp.sum_squares(jac @ delta_q - np.linalg.norm(dx)) + 0.0 * cp.sum_squares(delta_q))
    constraints = [delta_q <= 0.175, delta_q >= -0.175]
    prob = cp.Problem(objective=objective, constraints=constraints)
    prob.solve(solver=cp.SCS, verbose=False)
    q_ref = q + delta_q.value
    qdd0 = -Knull*(q - q_ref) - Dnull*dq

    contact = data.qfrc_constraint.reshape(-1,1)
        
    objective = cp.Minimize(cp.square(cp.norm(jac @ qdd + dJ_dt @ dq - mu_des)) + 0.02 * cp.square(cp.norm(u,1)) + 0 * cp.square(cp.norm(qdd)) 
                            + 0.5 * cp.square(cp.norm(qdd_null - N @ qdd0)))

    constraints = [pinv_Bp @ (M @ qdd + data.qfrc_bias.reshape(-1,1) - data.qfrc_passive.reshape(-1,1)) == u,
                np.zeros((nu,1)) >= u,
                u >= -100.0
                ]
