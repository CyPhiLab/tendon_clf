...
def controller(model, data, invariants, sigma, previous_solution=None):
    ...
    # If you have a posture reference q_ref:
    q = data.qpos.reshape(-1,1)
    # ----------------------------
    # 2) Zero reference
    delta_q = cp.Variable(shape=(model.nv, 1))
    objective = cp.Minimize(cp.sum_squares(jac @ delta_q - np.linalg.norm(dx)))
    ...
    objective = cp.Minimize(cp.square(cp.norm(dJ_dt @ dq + jac @ qdd - (K * twist.reshape(-1,1)  - 2 * np.sqrt(K) * (jac @ dq)))) + 0.2 * cp.square(cp.norm(qdd)) 
                            + 0.5 * cp.square(cp.norm(u,1)) + 1000 * cp.square(dl) + 1.5 * cp.sum_squares(qdd_null - qdd_0)) 
    constraints = [ 
                    dV <= - 1/e * V + dl, 
