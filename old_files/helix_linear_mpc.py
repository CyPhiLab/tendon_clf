...
def controller(model, data, invariants, previous_solution=None):
    ...
    nu = 9
    nq = model.nq
    qdd  = cp.Variable((nq, 1))  # joint accelerations
    ...
    q = data.qpos
    dq = data.qvel
    u_input = u_k.value.copy() if u_k.value is not None else None
    ...
    try:
        ...
        if u_k.value is not None:
            ...
            # print(f"converged\n")
            return task_error, q, dq, current_solution, u_input
        else:
