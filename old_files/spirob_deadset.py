...
def controller(model, data, invariants, previous_solution=None, trajectory=None):
    ...
    # ----------------------------
    # Null-space stabilizer (keep simple, no IK)
    # ----------------------------
    Knull = 200.0
    Dnull = 10 * np.sqrt(Knull)
    # reference posture = current posture => (q - q_ref)=0, so just add damping in null
    qdd_0 = -Dnull * dq

    # ----------------------------
    # Manipulability optimization (no IK)
    # ----------------------------
    # gradient of s(q)=0.5 logdet(JpJp^T + delta I)
    g_manip = manipulability_grad_fd(model, data, site_id, eps=1e-4, delta=1e-6)  # (nv,1)

    # project gradient into null space to avoid affecting task (approx)
    g_manip_null = N @ g_manip

    alpha_manip = 0.5  # tune: 1e-3 .. 1e-1
    ...
    objective = cp.Minimize(
        ...
        + 1000 * cp.square(dl)
        + 0 * cp.sum_squares(qdd_null - qdd_0)
        - 0 * (g_manip_null.T @ qdd)   # <-- manipulability reward
    )
    ...
    try:
        ...
        if u.value is not None:
            ...
            return V, task_error, q, dq_out, current_solution
        else:
            print("failed convergence - no solution\n")
            return V, task_error, q, dq_out, previous_solution
...
def simulate_model(headless=False, record_video=False, video_fps=30):
    ...
    if headless:
        ...
        while (len(task_error_log) < 2 or
            ...
            V, task_error, q, dq, previous_solution = controller(model, data, invariants, previous_solution=previous_solution, trajectory=trajectory)
