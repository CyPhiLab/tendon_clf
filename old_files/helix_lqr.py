...
def controller(model, data, invariants, previous_solution=None, S=None):
    ...
    # Synergy matrix
    S = compute_fixed_S_from_qbar(model, data, site_id, task_dim=6)
    # Task-space inertia 
...
def simulate_model(headless=False):
    ...
    if headless:
        ...
        while data.time < max_sim_time:
            ...
            task_error, q, dq, previous_solution, u = controller(model, data, invariants, previous_solution, S=None)
...
if __name__ == "__main__":
    ...
    csv_path = "helix_clf_qp_pos4.csv"
