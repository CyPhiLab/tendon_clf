...
def controller(model, data, invariants, previous_solution=None, trajectory=None):
    ...
    eta = np.concatenate((-twist,jac @ data.qvel - vel))
    V = eta.T @ Pe @ eta
    ...
    objective = cp.Minimize(cp.square(cp.norm(dJ_dt @ dq + jac @ qdd - acc.reshape(-1,1) - mu_des)) 
                            + 0.5 * cp.square(cp.norm(qdd))  + 0.5 * cp.square(cp.norm(u)) + 1000 * cp.square(dl) + 0.5 * cp.square(cp.norm(qdd_null)))  
...
def simulate_model(headless=False):
    ...
    model.jnt_stiffness[range(2,len(data.qpos),3)] = 2
...
if __name__ == "__main__":
    ...
    csv_path = "helix_id_clf_qp_tracking.csv"
