...
def controller(model, data, invariants, previous_solution=None, trajectory=None):
    ...
    dV = eta.T @ (F.T @ Pe + Pe @ F) @ eta + 2 * eta.T @ Pe @ G @ (mu - acc.reshape(-1,1))

    mu_des = (500 * twist.reshape(-1,1)  - 2 * np.sqrt(500) * (vel - jac @ dq))

    qdd = np.linalg.pinv(jac) @ (mu - dJ_dt @ dq)

    objective = cp.Minimize(0.2*cp.square(cp.norm(mu - acc.reshape(-1,1) - mu_des))  + 1000 * cp.square(dl)  + 0.02 * cp.square(cp.norm(u)))# + 0.2 * cp.square(cp.norm(qdd)))
...
if __name__ == "__main__":
    ...
    csv_path = "helix_clf_qp_tracking.csv"
