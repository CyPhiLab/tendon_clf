...
class ControlNode(Node):
    def __init__(self):
        ...
        # Control gains 
        self.K = self.declare_parameter('K', 500.0).get_parameter_value().double_value
        self.reg_qdd = self.declare_parameter('reg_qdd', 0.5).get_parameter_value().double_value
        self.reg_u = self.declare_parameter('reg_u', 0.5).get_parameter_value().double_value
        self.reg_dl = self.declare_parameter('reg_dl', 1000.0).get_parameter_value().double_value

        # Control input bounds 
        self.u_min = self.declare_parameter('u_min', -1.0).get_parameter_value().double_value
        self.u_max = self.declare_parameter('u_max', 0.0).get_parameter_value().double_value

        # Motor ids, in the same order as the u vector
        self.motor_ids = self.declare_parameter('motor_ids', [0, 1, 2]).get_parameter_value().integer_array_value

        # Control loop rate
        self.rate_hz = self.declare_parameter('rate_hz', 500.0).get_parameter_value().double_value

        # Fixed task-space target 
        target = self.declare_parameter('target_pos', [0.2, 0.0, 0.2]).get_parameter_value().double_array_value
        self.target_pos = np.array(target, dtype=float)

        # Name of the end-effector site in the MuJoCo model
        site_name = self.declare_parameter('site_name', 'ee').get_parameter_value().string_value
        ...
        self.state_sub = self.create_subscription(
            RobotState, '/spirob/robot_state', self._on_state, 1)

        # Create publisher for MotorCommand
        self.cmd_pub = self.create_publisher(MotorCommand, '/spirob/motor_command', 10)
