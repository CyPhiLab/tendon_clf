...
gear = 435.0  # Gear ratio for the actuators
...
def controller(model, data, invariants, sigma, previous_solution=None):
    ...
    z = cp.Variable(nu, boolean=True)  # binary: z_i=1 means tendon i is active
    twist[3:] = 0.0
    ...
    f = Mx @ (ydd - dJ_dt @ dq) + Jbar.T @ (data.qfrc_bias.reshape(-1,1) - data.qfrc_passive.reshape(-1,1))
    N = I - jac.T @ Jbar.T
    tau_null = np.linalg.pinv((Ip - I) @ N, rcond=1e-12) @ (I - Ip) @ (jac.T @ Mx @ (ydd - dJ_dt @ dq) 
                                                                        + jac.T @ Jbar.T @ (data.qfrc_bias.reshape(-1,1) - data.qfrc_passive.reshape(-1,1)))
    tau = jac.T @ Mx @ (ydd - dJ_dt @ dq) + jac.T @ Jbar.T @ (data.qfrc_bias.reshape(-1,1) - data.qfrc_passive.reshape(-1,1)) + N @ tau_null
    ...
    objective = cp.Minimize(cp.square(cp.norm(ydd - (K * twist.reshape(-1,1)  - 2 * np.sqrt(K) * (jac @ dq)))) 
                            + 0.02 * cp.square(cp.norm(qdd)) 
                            + 0.05 * cp.square(cp.norm(u,1)) 
                            + 1000 * cp.square(dl))  
    
    u_prev = data.ctrl.reshape(-1, 1)

    constraints = [ 
                    dV <= - 1/e * V + dl, 
                    pinv_Bp @ tau == u,
                    # pinv_Bp @ (M @ qdd + data.qfrc_bias.reshape(-1,1) - data.qfrc_passive.reshape(-1,1)) == u,
                    # np.linalg.pinv(jac @ M_inv @ B) @ jac @ M_inv @ jac.T @ f == u,
                    np.zeros((nu,1)) >= u,
                    u >= -1.0,
                    # cp.abs(u - u_prev) <= 0.05,
                    ]
    ...
    try:
        ...
        if u.value is not None:
            data.ctrl = np.clip(np.squeeze(u.value), -1.0, 0) 
...
def simulate_model(headless=False, record_video=False, video_fps=30):
    ...
    if record_video:
        ...
        camera.lookat[:] = [0.2, 0.0, 0.2]  # Look at robot center
        camera.azimuth = 90  # Side angle view
        camera.elevation = -0  # Slightly above
...
if __name__ == "__main__":
    ...
    for i in range(len(actuated_indices)):
        ...
        axs[i].axhline(control_limit*gear, color='r', linestyle='--', linewidth=1)
        axs[i].axhline(-control_limit*gear, color='r', linestyle='--', linewidth=1)
