...
def controller(model, data, invariants, sigma, previous_solution=None):
    ...
    dx = pos4 - data.site(site_id).xpos
    ...
    qdd_null = N @ qdd
    objective = cp.Minimize(cp.square(cp.norm(dJ_dt @ dq + jac @ qdd - (K * twist.reshape(-1,1)  - 2 * np.sqrt(K) * (jac @ dq)))) 
                            + 1.5 * cp.square(cp.norm(qdd)) 
                            + 0.5 * cp.square(cp.norm(u,1)) 
                            + 0.5 * cp.square(cp.norm(qdd - qdd_ref))
                            + 1000 * cp.square(dl))  # Add manipulability objective
    constraints = [ 
                    dV <= - 1/e * V + dl, 
